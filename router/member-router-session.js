...
router.post('/sign', isGuest, async (req, res, next) => {
	...
	if(result[0]){
		...
		if(match) {
			// 세션 구현
			req.session.user = { userid, id: result[0].id, username: result[0].username, email: result[0].email, grade: result[0].grade};
			sql = 'INSERT INTO loginlog SET uid=?, ip=?';
			sqlVal = [result[0].id, getIP(req)];
			result = await queryExecute(sql, sqlVal);
			res.send(alert('로그인 되었습니다.', '/') );
		}
		else res.send( alert('아이디 또는 패스워드가 올바르지 않습니다.', '/member/login') );
	}
	else{
		res.send( alert('아이디 또는 패스워드가 올바르지 않습니다.', '/member/login') );
	}
});
